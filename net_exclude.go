...
func (net *Net) Exclude(exclude_net *Net) {
  if net.Contains(exclude_net) {
    nets := net.Split()
    for _, part := range nets {
      part.Exclude(exclude_net)
    }
  } else {
    fmt.Println(net)
  }
...
func main() {
  ...
  exclude_net_m := net_regex.FindStringSubmatch(os.Args[2])
  if len(exclude_net_m) != 3 {
    fmt.Fprintln(os.Stderr, "Bad network: "+os.Args[2])
    os.Exit(1)
  }
  ...
  if !exclude_net.IsValid() {
    fmt.Fprintln(os.Stderr, "Invalid network/mask: "+os.Args[2])
    os.Exit(1)
  }
